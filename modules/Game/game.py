...
def slot_game(account_value: float) -> list:
    ...
    while True:
        try:
            ...
            if first_value in options and second_value in options and third_value in options:
                break
            else:
                print("\nYou must enter only the values ​​from the menu", end="\n\n")
        except ValueError as error:
    ...
    if (equal_value == 3):
        ...
        print("It will be considered a victory for the player, but he will not gain any monetary value", end="\n\n")
    else:
        account_value -= amount_wagered
        result = "v"
        print("The player lost")
...
def blue_jack(account_value: float) -> list:
    ...
    print(f"The cards for the player are: {first_value_player}, {second_value_player}, Sum: {first_value_player + second_value_player}", end="\n\n")
    ...
    while True:
        try:
            ...
            print(f"An error has occurred: {error}")
            print("You must enter only the values ​​from the menu", end="\n\n")
    third_value_player = 0 if (option_selected_by_player == "1") else random.randint(2,12)
